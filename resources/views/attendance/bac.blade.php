...
document.addEventListener('DOMContentLoaded', function() {
  ...
  form.addEventListener('submit', async function(e) {
    ...
    if (!form.fullName.value || !form.position.value ||
        selectedTypeElem.textContent.trim() === 'Select Attendee Type' ||
        !form.phone_number.value || !form.attendance_date.value) {
      Swal.fire({
        icon: 'warning',
        title: 'Oops!',
    ...
    if (!capturedPhoto.src) {
      Swal.fire({
        icon: 'warning',
        title: 'Oops!',
        text: 'Please capture your photo!',
      });
      return;
    }
    ...
    try {
      ...
      formData.set('photo', capturedPhoto.src); // base64 image
      ...
      if (result.success) {
        ...
        selectedTypeElem.classList.add('text-gray-500');
        selectedTypeElem.classList.remove('text-gray-800');
        capturedPhoto.src = '';
        capturedPhoto.classList.add('hidden');
